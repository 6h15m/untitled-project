...
router.post('/', async function (req, res, next) {
  ...
  try {
    const { small_category_id, product_name, product_price, tags, options }: PostCreateType = req.body;
    ...
    await each(async (tag) => {
      tag.name ? await QUERY`INSERT INTO tags (id, name) VALUES (${tag.id}, ${tag.name})` : '';
    }, toAsync(tags));
    const product_id = (await QUERY`SELECT id FROM products ORDER BY id DESC LIMIT 1`)[0].id;
    await each(async (tag) => {
    ...
    await each(async (option) => {
      ...
      let option_id = (await QUERY`SELECT id FROM options ORDER BY id DESC LIMIT 1`)[0].id;
